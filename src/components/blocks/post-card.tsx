...
export function PostCard({ post, className }: PostCardProps) {
  return (
    <Card className={cn("h-full flex flex-col overflow-hidden hover:shadow-lg transition-shadow duration-300 p-0", className)}>
      ...
      <CardHeader>
        <div className="text-sm text-muted-foreground mb-2">{format(new Date(post.publishedAt), "MMMM d, yyyy")}</div>
      ...
      <CardFooter className="p-4 pt-0">
        <Button asChild variant="outline" className="w-full">
          <Link href={`/blog/${post.page.slug}/${post.slug}`}>Read More</Link>
        </Button>
      </CardFooter>
    </Card>
  );
